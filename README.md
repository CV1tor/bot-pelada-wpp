...
`!votacao Nome` envia uma enquete com as opções de 1 a 5 estrelas, seleção única e janela de 24 horas. `!votacao` sem argumentos, ou `!votacao todos`, abre uma enquete para cada nome confirmado na lista. O bot persiste o ID e o `messageSecret` retornados. Atualizações agregadas da Evolution API, opções nominais e atualizações binárias com o hash SHA-256 da opção são aceitas.
